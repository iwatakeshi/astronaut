...
type ClassFormatter = (value: string | number, prefix: string, breakpoint: string) => string;
interface Options {
...
export class ClassList {
  static fromResponsiveValue<T extends string | number>(prefix: string, value: ResponsiveValue<T> | undefined, { format: formatter, filter }: Options = {}) {
    if (value === undefined) return "";
    const $formatter = formatter ? formatter : (value: string | number, prefix: string, breakpoint: string) => {
      if (['base', '_'].includes(breakpoint)) return `${prefix}-${value}`;
